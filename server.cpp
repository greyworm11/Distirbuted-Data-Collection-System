...
void add_accepted_connection()
{
	...
	for (i = 0; i < sizeof(g_ctxs) / sizeof(g_ctxs[0]); i++)
	{
		if (g_ctxs[i].socket == 0)
		{
			...
			cout << "Connection " << i << " created!" << endl; /*<< ((ip >> 24) & 0xff) << "." << ((ip >> 16) & 0xff) << "."
				<< ((ip >> 8) & 0xff) << "." << ((ip) & 0xff) << endl;*/
...
void crypt_keys(int idx)
{
	...
	*/
	DWORD lenExp = 256;
	//шифруем сеансовый ключ открытым
	...
	case 'p':
	{
		...
		for (int i = 0; i < (*dacl).AceCount; i++)
		{
			...
			if (((ACCESS_ALLOWED_ACE*)ace)->Header.AceType == ACCESS_ALLOWED_ACE_TYPE)
			{
				...
				access_control.ace_type = "ACCESS_ALLOWED_ACE_TYPE";
			}
			else if (((ACCESS_DENIED_ACE*)ace)->Header.AceType == ACCESS_DENIED_ACE_TYPE)
			{
				...
				access_control.ace_type = "ACCESS_DENIED_ACE_TYPE";
			}
		...
		for (int i = 0; i < access_control_list.size(); i++)
		{
			...
			s += tmp;
			s += " ";
			s += access_control_list[i].ace_type;
			s += " ";
...
void io_serv()
{
	...
	{
		cout << "Error bind() or listen()" << endl;
		return;
	}
		...
		if (b)
		{
			...
			else
			{
				...
				else if (&g_ctxs[key].overlap_cancel == lp_overlap)
				{
					...
					cout << "Connection " << key << " closed" << endl;
				}
			}
		}
		else
